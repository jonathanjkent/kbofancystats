...
kable(d, digits = c(0,0,3,2,1,1,1,1,1,1,1), row.names = T, col.names = c("Team","Elo", "wOBA", "FIP","SFR", "1st","2nd","3rd","4th","5th","Out")) %>%
  add_header_above(c("Current Elos & Stats" = 6, "2022 Playoff Seed Likelihood (%)" = 6)) %>% kable_styling(bootstrap_options = c("striped", "hover"), fixed_thead = T) %>% column_spec(2, bold = T) %>% column_spec(6, border_right = T)
```
