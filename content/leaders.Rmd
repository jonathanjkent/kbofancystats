...
### xFIP {#xfip}
```{r, echo=F,warning=FALSE,message=FALSE}
d <- stats %>% filter(pitIp2 > limit) %>% select(playerId,playerNameKor,Name,TeamNameEng,age,position,xFIP) %>% arrange(xFIP)
d <- d[1:10,]
d$Name <- paste0("[", d$Name," | ",d$playerNameKor, "](/players/", d$playerId, ")")
d$TeamNameEng <- paste0("[", d$TeamNameEng, "](/teams/", gsub("[[:space:]]", "", tolower(d$TeamNameEng)), ")")
d <- d %>% select(-playerId,-playerNameKor)
kable(d, digits = c(0,0,0,0,2), row.names = T, col.names = c("Name","Team","Age","Pos","xFIP")) %>% kable_styling(bootstrap_options = c("striped", "hover"), fixed_thead = T) %>% column_spec(2, bold = T, border_right = F)
```
***
### K/9 {#k9}
```{r, echo=F,warning=FALSE,message=FALSE}
