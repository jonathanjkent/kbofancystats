---
date: "2021-03-31"
summary: 'WAR Leader | José Miguel Fernández, IF: 2.31'
title: 'Doosan Bears'
subtitle: 'Seoul | Current Elo: 1513 (5)'
image:
  preview_only: true
---
**2021** | [Batting](#batting) | [Pitching](#pitching) | 
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/DoosanBears/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/DoosanBears/#pitching) | 
...
d <- batters %>% filter(Team == "Doosan Bears") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Doosan Bears") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
