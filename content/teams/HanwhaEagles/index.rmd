---
date: "2020-03-31"
summary: 'WAR Leader | Warwick Saupold, RHP: 1.84'
title: 'Hanwha Eagles'
...
d <- batters %>% filter(Team == "Hanwha Eagles") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Hanwha Eagles") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
