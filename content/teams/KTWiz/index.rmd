---
date: "2021-03-31"
summary: 'WAR Leader | Odrisamer Despaigne, RHP: 0.28'
title: 'KT Wiz'
subtitle: 'Suwon | Current Elo: 1516 (4)'
image:
  preview_only: true
---
**2021** | [Batting](#batting) | [Pitching](#pitching) | 
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/KTWiz/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/KTWiz/#pitching) | 
...
d <- batters %>% filter(Team == "KT Wiz") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "KT Wiz") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
