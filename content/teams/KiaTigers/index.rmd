---
date: "2021-03-31"
summary: 'WAR Leader | Daniel Mengden, RHP: 0.29'
title: 'Kia Tigers'
subtitle: 'Gwangju | Current Elo: 1491 (7)'
image:
  preview_only: true
---
**2021** | [Batting](#batting) | [Pitching](#pitching) | 
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/KiaTigers/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/KiaTigers/#pitching) | 
...
d <- batters %>% filter(Team == "Kia Tigers") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Kia Tigers") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
