---
date: "2021-03-31"
summary: 'WAR Leader | Seo Geon-chang, 2B: 0.68'
title: 'Kiwoom Heroes'
subtitle: 'Seoul | Current Elo: 1499 (5)'
image:
  preview_only: true
---
**2021** | [Batting](#batting) | [Pitching](#pitching) | 
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/KiwoomHeroes/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/KiwoomHeroes/#pitching) | 
...
d <- batters %>% filter(Team == "Kiwoom Heroes") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Kiwoom Heroes") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
