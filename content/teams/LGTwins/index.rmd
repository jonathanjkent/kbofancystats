---
date: "2021-03-31"
summary: 'WAR Leader | Andrew Suarez, LHP: 1.26'
title: 'LG Twins'
...
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/LGTwins/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/LGTwins/#pitching) | 
...
d <- batters %>% filter(Team == "LG Twins") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "LG Twins") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
