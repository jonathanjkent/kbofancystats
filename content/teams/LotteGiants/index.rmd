---
date: "2021-03-31"
summary: 'WAR Leader | Dan Straily, RHP: 2.34'
title: 'Lotte Giants'
subtitle: 'Busan | Current Elo: 1481 (8)'
image:
  preview_only: true
---
**2021** | [Batting](#batting) | [Pitching](#pitching) | 
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/LotteGiants/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/LotteGiants/#pitching) | 
...
d <- batters %>% filter(Team == "Lotte Giants") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Lotte Giants") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
