---
date: "2021-03-31"
summary: 'WAR Leader | Park Jong-hun, RHP: 0.38'
title: 'SSG Landers'
subtitle: 'Incheon | Current Elo: 1472 (9)'
image:
  preview_only: true
---
**2021** | [Batting](#batting) | [Pitching](#pitching) | 
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/SSGLanders/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/SSGLanders/#pitching) | 
...
d <- batters %>% filter(Team == "SSG Landers") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "SSG Landers") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
