---
date: "2021-03-31"
summary: 'WAR Leader | David Buchanan, RHP: 4.32'
title: 'Samsung Lions'
subtitle: 'Daegu | Current Elo: 1513 (4)'
image:
  preview_only: true
---
**2021** | [Batting](#batting) | [Pitching](#pitching) | 
**2020** | [Batting](https://www.kbofancystats.com/teams-2020/SamsungLions/#batting) | [Pitching](https://www.kbofancystats.com/teams-2020/SamsungLions/#pitching) | 
...
d <- batters %>% filter(Team == "Samsung Lions") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Samsung Lions") %>% filter(Year == "2021") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
