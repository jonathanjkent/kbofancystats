...
| [BABIP](#babip) | [BB% & SO%](#bbso) | [BB/9, K/9 & HR/9](#bbkhr) | [ISO](#iso) | [FIP](#fip) | [OPS](#ops) | [wRC+](#wrc) | [WAR](#war) |
...
batters <- batters %>% subset(Season == "2019") %>% subset(PA > 446) %>% select(BABIP, BBpct, SOpct, ISO, OPS, WAR)
pitchers <- pitchers %>% subset(Season == "2019") %>% subset(IP2 > 144) %>% select(BABIP, BB9, SO9, HR9, FIP, WAR)
...
Distribution of WAR for all qualifying batters and pitchers since 1999.
```{r, echo=F,warning=FALSE,message=FALSE}
batters <- read_csv("~/Google Drive/KBO/batters_simple.csv")
pitchers <- read_csv("~/Google Drive/KBO/pitchers_simple.csv")
...
p + theme_minimal() + labs(color = "Proportion of Players", x = "WAR", y = "")
```
