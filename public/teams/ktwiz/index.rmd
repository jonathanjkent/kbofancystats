---
date: "2020-03-31"
summary: 'WAR Leader | Odrisamer Despaigne, RHP: 1.24'
title: 'KT Wiz'
subtitle: 'Suwon | Current Elo: 1500 (5)'
image:
  preview_only: true
---
...
d <- batters %>% filter(Team == "KT Wiz") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "KT Wiz") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
