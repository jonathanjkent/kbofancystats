---
date: "2020-03-31"
summary: 'WAR Leader | Dan Straily, RHP: 0.71'
title: 'Lotte Giants'
subtitle: 'Busan | Current Elo: 1465 (10)'
image:
  preview_only: true
---
...
d <- batters %>% filter(Team == "Lotte Giants") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Lotte Giants") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
