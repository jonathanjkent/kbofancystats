---
date: "2020-03-31"
summary: 'WAR Leader | Koo Chang-mo, LHP: 0.72'
title: 'NC Dinos'
...
d <- batters %>% filter(Team == "NC Dinos") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "NC Dinos") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
