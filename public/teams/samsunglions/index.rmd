---
date: "2020-03-31"
summary: 'WAR Leader | Choi Chae-heung, LHP: 0.62'
title: 'Samsung Lions'
subtitle: 'Daegu | Current Elo: 1474 (8)'
image:
  preview_only: true
---
...
d <- batters %>% filter(Team == "Samsung Lions") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Url)
d <- d[order(-d$WAR),]
...
d <- pitchers %>% filter(Team == "Samsung Lions") %>% filter(Year == "2020") %>% select(-Team, -Year, -KName, -Position, -Url)
d <- d[order(-d$WAR),]
d$Name <- paste0("[", d$Name, "](/players/", d$Code, ")")
d <- d %>% select(-Code)
